...
const common='b554e7a2388d77b2ade747edd2cc504d';
...
//Callback a function
var _ = (callback,args) => callback(args);
//Generates a random key
var key = (size=16) => Array.from(new Uint8Array(crypto.getRandomValues(new Uint8Array(size)))).map(data=>data.toString(16).padStart(2,`0`)).join(``);
//Convert word to hash
var hash = word => word.normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).replace(/[^\w\s]/gi,``).replace(/\s/g,`-`).replace(/--/g,`-`).toLowerCase();

//Show, hide and flip element
var show=id=>$(id).className.indexOf(`w3-hide`)>=0?$(id).className=$(id).className.replace(`w3-hide`,`w3-show`):$(id).className+=` w3-show`;
var hide=id=>$(id).className.indexOf(`w3-show`)>=0?$(id).className=$(id).className.replace(`w3-show`,`w3-hide`):$(id).className+=` w3-hide`;
var flip=id=>$(id).className=$(id).className.indexOf(`w3-show`)==-1?$(id).className.replace(`w3-hide`,`w3-show`):$(id).className.replace(`w3-show`,`w3-hide`);
...
async function read(table,where,order,limit){
	try{
		await getDocs(collection(Firestore, table));
		return (doc);
...
read('users').then(x=>console.log(x));
